...
public class SerieDescripLargeAdapter extends RecyclerView.Adapter<SerieDescripLargeAdapter.TVShowViewHolder> {

    private Context mContext;
    private List<SerieDescrip> mSeries;

    public SerieDescripLargeAdapter(Context context, List<SerieDescrip> Series) {
        mContext = context;
        mSeries = Series;
    }
    ...
    public void onBindViewHolder(TVShowViewHolder holder, int position) {
        ...
        if (mSeries.get(position).getName() != null)
            holder.tvShowTitleTextView.setText(mSeries.get(position).getName());
        else
            holder.tvShowTitleTextView.setText("");

        if (mSeries.get(position).getVoteAverage() != null && mSeries.get(position).getVoteAverage() > 0) {
            holder.tvShowRatingTextView.setVisibility(View.VISIBLE);
            holder.tvShowRatingTextView.setText(String.format("%.1f", mSeries.get(position).getVoteAverage()) + Constants.RATING_SYMBOL);
        } else {
            holder.tvShowRatingTextView.setVisibility(View.GONE);
        }

        setGenres(holder, mSeries.get(position));

        if (Favoris.isTVShowFav(mContext, mSeries.get(position).getId())) {
    ...
    private void setGenres(TVShowViewHolder holder, SerieDescrip tvShow) {
        String genreString = "";
        for (int i = 0; i < tvShow.getGenreIds().size(); i++) {
            if (tvShow.getGenreIds().get(i) == null) continue;
            if (SeriesGenres.getGenreName(tvShow.getGenreIds().get(i)) == null) continue;
            genreString += SeriesGenres.getGenreName(tvShow.getGenreIds().get(i)) + ", ";
        }
        ...
        public TVShowViewHolder(View itemView) {
            ...
            tvShowCard.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    ...
                    intent.putExtra(Constants.TV_SHOW_ID, mSeries.get(getAdapterPosition()).getId());
            ...
            tvShowFavImageButton.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    ...
                    Favoris.addTVShowToFav(mContext, mSeries.get(getAdapterPosition()).getId(), mSeries.get(getAdapterPosition()).getPosterPath(), mSeries.get(getAdapterPosition()).getName());
